...
impl ProvisioningCmd {
    pub async fn run(self, udid: Option<String>, json: bool) -> Result<()> {
        ...
        let stream = device
            .connect_service(ios_services::misagent::SERVICE_NAME)
            .await?;
        let mut client = ios_services::misagent::MisagentClient::new(stream);
...
fn profile_to_json(profile: &ios_services::misagent::Profile) -> serde_json::Value {
    serde_json::json!({
        "uuid": profile.uuid,
        "name": profile.name,
...
fn find_profile<'a>(
    profiles: &'a [ios_services::misagent::Profile],
    query: &str,
) -> Result<&'a ios_services::misagent::Profile> {
...
fn unique_profile_match<'a, F>(
    ...
    query: &str,
    predicate: F,
) -> Result<Option<&'a ios_services::misagent::Profile>>
where
    F: Fn(&ios_services::misagent::Profile) -> bool,
{
...
fn print_profiles(profiles: &[ios_services::misagent::Profile]) {
...
fn print_profile_details(profile: &ios_services::misagent::Profile) {
    for (label, value) in profile_detail_lines(profile) {
        println!("{label:<12} {value}");
    }
}

fn profile_detail_lines(profile: &ios_services::misagent::Profile) -> Vec<(&'static str, String)> {
    vec![
