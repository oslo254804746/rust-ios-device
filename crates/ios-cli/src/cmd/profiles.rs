...
fn print_profiles(profiles: &[ios_core::mcinstall::ProfileInfo]) {
...
fn print_profile_details(profile: &ios_core::mcinstall::ProfileInfo) {
    for (label, value) in profile_detail_lines(profile) {
        println!("{label:<19} {value}");
    }
}

fn profile_detail_lines(profile: &ios_core::mcinstall::ProfileInfo) -> Vec<(&'static str, String)> {
    let mut lines = vec![
...
fn profile_matches_query(profile: &ios_core::mcinstall::ProfileInfo, query: &str) -> bool {
...
mod tests {
    ...
    fn print_profiles_includes_extended_metadata_columns() {
        let output = {
            let profiles = vec![ios_core::mcinstall::ProfileInfo {
    ...
    fn print_profile_details_includes_extended_fields() {
        let profile = ios_core::mcinstall::ProfileInfo {
            identifier: "com.example.profile".into(),
            display_name: "Example".into(),
            description: Some("Example description".into()),
            is_active: true,
