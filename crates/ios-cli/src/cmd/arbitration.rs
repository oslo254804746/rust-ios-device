...
impl ArbitrationCmd {
    pub async fn run(self, udid: Option<String>, json: bool) -> Result<()> {
        ...
        let mut client = ios_services::arbitration::ArbitrationClient::new(stream);
...
async fn connect_arbitration(udid: &str) -> Result<ios_core::device::ServiceStream> {
    ...
    if should_use_tunnel_for_arbitration(version.major) {
        ...
        device
            .connect_rsd_service(ios_services::arbitration::SERVICE_NAME)
            .await
            .map_err(|err| {
                anyhow::anyhow!(
                    "arbitration tunnel fallback reached RSD, but '{}' is not exposed there: {err}",
                    ios_services::arbitration::SERVICE_NAME
                )
            })
    } else {
        let device = connect(
        ...
        .await?;
        device
            .connect_service(ios_services::arbitration::SERVICE_NAME)
            .await
            .map_err(Into::into)
    }
}
