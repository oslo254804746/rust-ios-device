...
async fn list_files_sends_list_command_and_decodes_response() {
    ...
    let mut client = ios_services::fetchsymbols::FetchSymbolsClient::new(&mut stream);
...
async fn remote_list_files_bootstraps_xpc_and_decodes_catalog() {
    ...
    let server_task = tokio::spawn(async move {
        ...
        assert!(matches!(
            ...
            Some(ios_xpc::XpcValue::Array(paths)) if paths.is_empty()
        ));
        assert!(matches!(
            request_dict.get("XPCDictionary_sideChannel"),
            Some(ios_xpc::XpcValue::Uuid(_))
        ));

        write_xpc_response(
            &mut server,
            3,
            ios_xpc::XpcValue::Dictionary(IndexMap::from([(
                "DSCFilePaths".to_string(),
                ios_xpc::XpcValue::Uint64(2),
            )])),
        )
        .await;
    ...
    let mut client = ios_services::fetchsymbols::RemoteFetchSymbolsClient::connect(client)
        .await
...
async fn remote_download_opens_file_stream_and_copies_bytes() {
    ...
    let server_task = tokio::spawn(async move {
        ...
        assert_eq!(
            open.flags,
            ios_xpc::message::flags::ALWAYS_SET | ios_xpc::message::flags::FILE_TX_STREAM_RESPONSE
        );
...
fn remote_catalog_entry(path: &str, size: u64) -> ios_xpc::XpcValue {
    ios_xpc::XpcValue::Dictionary(IndexMap::from([(
        "DSCFilePaths".to_string(),
        ios_xpc::XpcValue::Dictionary(IndexMap::from([
            (
                "filePath".to_string(),
                ios_xpc::XpcValue::String(path.to_string()),
            ),
            (
                "fileTransfer".to_string(),
                ios_xpc::XpcValue::FileTransfer {
                    msg_id: 0,
                    data: Box::new(ios_xpc::XpcValue::Dictionary(IndexMap::from([(
                        "s".to_string(),
                        ios_xpc::XpcValue::Uint64(size),
                    )]))),
                },
            ),
        ])),
...
fn remote_catalog_entry_nested(path: &str, size: u64, msg_id: u64) -> ios_xpc::XpcValue {
    ios_xpc::XpcValue::Dictionary(IndexMap::from([(
        "DSCFilePaths".to_string(),
        ios_xpc::XpcValue::Dictionary(IndexMap::from([
            (
...
async fn read_xpc_request<S>(stream: &mut S, stream_id: u32) -> ios_xpc::XpcMessage
where
    S: AsyncRead + AsyncWrite + Unpin,
{
    let frame = read_raw_frame(stream).await;
    ...
    assert_eq!(frame.stream_id, stream_id);
    ios_xpc::message::decode_message(bytes::Bytes::from(frame.payload)).unwrap()
}
...
async fn write_xpc_response<S>(stream: &mut S, stream_id: u32, body: ios_xpc::XpcValue)
where
    S: AsyncRead + AsyncWrite + Unpin,
{
    write_raw_frame(
    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
    write_raw_frame(
        ...
        &ios_xpc::message::encode_message(&ios_xpc::XpcMessage {
            flags: ios_xpc::message::flags::ALWAYS_SET
                | ios_xpc::message::flags::FILE_TX_STREAM_REQUEST,
            msg_id: 0,
            body: None,
        })
        .unwrap(),
