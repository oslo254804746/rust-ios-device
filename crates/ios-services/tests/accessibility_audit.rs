...
use ios_services::accessibility_audit::{
...
use ios_services::dtx::{encode_dtx, read_dtx_frame, DtxPayload, NSObject};
...
async fn explicit_publish_handshake_sends_capabilities_before_requesting_device_capabilities() {
    ...
    let task = tokio::spawn(async move {
        let mut audit = AccessibilityAuditClient::new_with_handshake(
            client,
            17,
            ios_services::accessibility_audit::AccessibilityAuditHandshake::PublishCapabilities,
        );
        audit.capabilities().await.unwrap()
    });
