...
use crate::dtx::types::DtxPayload;
    ...
    pub async fn sample(&mut self) -> Result<EnergySample, DtxError> {
        let response = self
            ...
            .method_call(
                ...
                &[
                    archived_object(ios_proto::nskeyedarchiver_encode::archive_dict(vec![])),
...
fn encode_pid_array(pids: &[i32]) -> Vec<u8> {
    ios_proto::nskeyedarchiver_encode::archive_array(
        pids.iter()
...
mod tests {
    ...
    fn encodes_pid_array() {
        ...
        let decoded = ios_proto::nskeyedarchiver::unarchive(&encoded).unwrap();
