...
use ios_xpc::h2_raw::H2Framer;
use ios_xpc::{XpcMessage, XpcValue};
...
impl<S: AsyncRead + AsyncWrite + Unpin> RestoreServiceClient<S> {
    ...
    async fn send_command(
        ...
        let request = ios_xpc::message::encode_message(&XpcMessage {
    ...
    fn try_take_pending_control_message(&mut self) -> Result<Option<XpcMessage>, RestoreError> {
        ...
        let message = ios_xpc::message::decode_message(payload).map_err(|err| {
    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
    framer
        .write_client_server(
            &ios_xpc::message::encode_message(&XpcMessage {
